...
(function (window) {
  ...
  (function () {
    ...
    function openLightbox(index) {
      if (!lb) buildDOM();
      current = Math.max(0, Math.min(index, pool.length - 1));
      showImage();
    ...
    function showImage() {
      ...
      lbPrev.style.display = pool.length > 1 ? 'flex' : 'none';
      lbNext.style.display = pool.length > 1 ? 'flex' : 'none';
    }
    ...
    window.initArticleLightbox = function (container) {
      ...
      // Collect all clickable images (body content + photo grids)
      // Exclude UI chrome: card thumbnails, avatars, hero behind a link, etc.
      var imgs = Array.prototype.slice.call(
        container.querySelectorAll('.reader-body-content img, .article-grid-img')
      );
      ...
      imgs.forEach(function (img) {
        ...
        // Find caption: check next figcaption sibling or parent figcaption
        var caption = '';
        ...
        if (!caption && img.alt && img.alt !== 'Photo' && img.alt !== 'Article image') {
          caption = img.alt;
        }
